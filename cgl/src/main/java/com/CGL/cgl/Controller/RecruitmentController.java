...
public class RecruitmentController {
    ...
    public ResponseEntity<RecruitmentRequest> submit(
        ...
        return ResponseEntity.status(201).body(
            service.submitRequest(request, email)
        );
    }
    ...
    public ResponseEntity<RecruitmentRequest> approveRequest(
        ...
        return ResponseEntity.ok(service.approveRequest(id, email));
    }
    ...
    public ResponseEntity<RecruitmentRequest> rejectRequest(
        ...
        return ResponseEntity.ok(service.rejectRequest(id, email));
    }
    ...
    @PreAuthorize("hasAnyRole('SUPER_ADMIN','DEPT_HEAD','CPSB_ADMIN')")
    public ResponseEntity<List<RecruitmentRequest>> getRequestsByDepartment(
        @PathVariable Long departmentId
    ) {
        return ResponseEntity.ok(service.getRequestsByDepartment(departmentId));
    }
}
